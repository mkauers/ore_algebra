...
def guess(data, algebra, **kwargs):
    ...
    if R.is_field():
        ...
        # CRA
        return _guess_via_hom(data, A, _word_size_primes(),
                              lambda mod: GF(mod), **kwargs)
...
def guess_hp(data, A, order=-1, degree=-1, lift=None, cut=25, ensure=0, infolevel=0):
    ...
    * if the condition on ``ensure`` is violated.
    ...
    return [(~L.leading_coefficient().leading_coefficient())*L for L in sol]
        ...
        if nn == 1:
            ...
            try:
                ...
                if abs(r2 - r1) >= 2 and abs(d2 - d1) >= 2:
                    path = [(i, d2 + ((d1-d2)*(i-r2))//(r1-r2))
                            for i in range(r2, r1, 1 if r1 >= r2 else -1)] + path
                    kwargs['path'] = path
                else:
        ...
        if ncpus == 1:
            ...
            for i in range(max(1, nn - 3)): # do several imgs before proceeding with a reconstruction attempt
                ...
                if not qq:
                    ...
                    qq = hom(qq[1])
                    Lp = guess(data_mod, OreAlgebra(hom(K.one()).parent()[x],
                                                    (A.var(), {x: qq * x}, {}),
                                                    q=qq), **kwargs)
            ...
            primes = [(p, hom, A.change_ring(hom(K.one()).parent()[x]))
                      for p, hom in primes]
            Lp = A.zero()
            p = K.one()
            out = [(arg[0][0], arg[0][2], Lpp)
                   for arg, Lpp in forked_guess(primes)]
...
def _guess_via_gcrd(data, A, **kwargs):
    ...
    if 'degree' in kwargs:
        max_deg = kwargs['degree']
        del kwargs['degree']
    elif 'max_degree' in kwargs:
        max_deg = kwargs['max_degree']
        del kwargs['max_degree']

    if 'min_degree' in kwargs:
        min_deg = kwargs['min_degree']
        del kwargs['min_degree']

    if 'order' in kwargs:
        max_ord = kwargs['order']
        del kwargs['order']
    elif 'max_order' in kwargs:
        max_ord = kwargs['max_order']
        del kwargs['max_order']
...
def guess_mult(data, algebra, **kwargs):
    ...
    for i in range_dim:
        if algebra.is_D(i):
            offset[i] = -ord[i]
            A.append(lambda n, u, v: (n - u + v, v))
            B.append(lambda n, u, v: n - u + v)
        elif algebra.is_S(i):
            A.append(lambda n, u, v: (n, u))
            B.append(lambda n, u, v: n + v)
        elif algebra.is_Q(i):
            _, q = algebra.is_Q(i)
            A.append(lambda n, u, v: (q, n*u))
            B.append(lambda n, u, v: n + v)
        else:
