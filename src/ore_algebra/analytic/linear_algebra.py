...
def orbit(Mats, vec, *, transition=False, pivots=False):
    r"""
    Return a basis of the smallest subspace containing ``vec`` and invariant
    under (the action of) the matrices of ``Mats``.
...
def GCD(a, b):
    ...
    Some words about the correction of this function: given a· and b· fixed, if 
    ``a`` and ``b`` are precise enough then GCD(``a``, ``b``) contains the gcd
    of a· and b·.
...
def squarefree_part(pol):
    ...
    Some words about the correction of this function: given pol· fixed, if 
    ``pol`` is precise enough, then squarefree_part(``pol``) contains the
    squarefree part of pol·.
...
def roots(pol, *, multiplicities=False):
    ...
    A list of complex numbers or a list of couples (r, m) with r a complex 
    number and m a positive integer if multiplicities=``True`` is specified.
...
def eigenvalues(mat, multiplicities=False):
    ...
    See the function :func:`roots` for more details.
    """
