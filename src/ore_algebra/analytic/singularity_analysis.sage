...
from ore_algebra.analytic.bounds import DiffOpBound
from ore_algebra.analytic.ui import multi_eval_diffeq
...
def _der_expf_R(k, R):
    ...
    Find an expression for [(d/dx)^k exp(f(x))]/exp(f(x)) given in a polynomial ring R
    """
    tup_f_k = R.gens()
    if k==0:
        return R(1)
...
def truncated_psi(n, m, v, logz):
    """
    Compute psi^(m)(z) truncated at z^(-m-2n-1) with error bound of order z^(-m-2n-2)
    ...
    """
    R = v.parent()
    Enz_coeff = 2*abs(bernoulli(2*n+2)) * (m + 2*n + 2)^(m + 2*n + 2) / (2*n + 1)^(2*n + 1) / (m + 1)^(m + 1) * gamma(m+2) / (2*n + 1) / (2*n + 2)
    if m==0:
        return R(logz - v / 2 - sum(bernoulli(2*k)*v^(2*k)/(2*k) for k in range(1,n+1)) + CBF(0).add_error(Enz_coeff)*v^(2*n+2))
    else:
        return R((-1)^(m+1) * (gamma(m) * v^m + gamma(m+1) * v^(m+1) / 2 + sum(bernoulli(2*k)*v^(2*k+m)*rising_factorial(2*k+1, m-1) for k in range(1,n+1))) + CBF(0).add_error(Enz_coeff)*v^(2*n+m+2))

def truncated_logder(alpha, l, order, v, logz, min_n = None):
    """
    Find a truncated expression with error bound for [(d/dα)^l (Γ(n+α)/Γ(α))] / (Γ(n+α)/Γ(α))
    ...
    INPUT:
        ...
        - min_n : positive number where n >= min_n is guaranteed, min_n > -alpha needed

    OUTPUT:
        a polynomial in CBF[v] such that [(d/dα)^l (Γ(n+α)/Γ(α))] / (Γ(n+α)/Γ(α)) is in its range
        when n >= max(s*|alpha|, min_n)
    """
    list_f = []
    R = v.parent()
    for m in range(l):
        n = max(0, ceil((order - m - 1)/2))
        Enz_coeff = abs(bernoulli(2*n+2)) * (m + 2*n + 2)^(m + 2*n + 2) / (2*n + 1)^(2*n + 1) / (m + 1)^(m + 1) * gamma(m+2) / (2*n + 1) / (2*n + 2)
...
def truncated_ratio_gamma(alpha, order, u, s):
    ...
    OUTPUT:
        - ratio_gamma : a polynomial in CBF[u] such that Γ(n+α)/Γ(n+1)/(n+α/2)^(α-1) is in its range when n >= s*|alpha|
    """
    rho = CBF(alpha/2)
    ...
    ratio_gamma_asy = sum(CBF(binomial(2*j-2*vd, 2*j).subs({vd : rho}) * series_gen_bern[2*j]) * u^(2*j)  for j in range(n_gam))
    Rnw_bound = CBF(binomial(2*n_gam-2*vd, 2*n_gam).subs({vd : rho.real()})) * CBF(abs(series_gen_bern_abs[2*n_gam])) * CBF(abs(alpha.imag())/2/s).exp() * CBF((s+1/2)/(s-1/2)).pow(max(0, -2*rho.real()+1+2*n_gam))
...
def truncated_power(alpha, order, w, s):
    ...
    OUTPUT:
        - trunc_power : a polynomial in CBF[w] such that (1 + α/2n)^(α-1) is in its range when n >= s*|alpha|
    """
    alpha_CBF = CBF(alpha)
    Mr = CBF(abs(alpha)).pow(order+1) / (1 - 1/s) * CBF(abs(alpha.imag())/2).exp() * (CBF(3/2).pow(alpha.real() - 1) if alpha.real() - 1 > 0 else CBF(1/2).pow(alpha.real() - 1))
    P.<t> = CBF[]
...
def truncate_tail(f, deg, min_n, w):
    """
    ...
    for c, mon in f:
        deg_w = mon.degree(w)
        if deg_w > deg:
            tuple_mon_g = tuple(map(lambda x, y: x - y, mon.exponents()[0], (w^(deg_w - deg)).exponents()[0]))
            mon_g = prod(R.gens()[j]^(tuple_mon_g[j]) for j in range(len(tuple_mon_g)))
            c_g = (c if c.mid()==0 else CBF(0).add_error(c.above_abs())) / CBF(min_n^(deg_w - deg))
            g = g + c_g * mon_g
        else:
            g = g + c*mon
    return g
...
def bound_coeff_mono(alpha, l, deg, w, logn, s=5, min_n=50):
    ...
    INPUT:
        ...
        - logn : element of the same polynomial ring as w, variable representing log(n)
        - s : positive number where n >= s*|alpha| is guaranteed, s > 2
        - min_n : positive number where n >= min_n is guaranteed, min_n > -alpha needed
    ...
    if not (QQbar(alpha).is_integer() and QQbar(alpha) <= 0):
        c = CBF(1/gamma(alpha)) #Value of 1/Γ(α)
        f = truncated_ratio_gamma(alpha, order, u, s) #Bound for (n+α/2)^(1-α) * Γ(n+α)/Γ(n+1)
        bound_error_u = CBF(abs(alpha/2)^order / (1 - 1/(2*s)))
        truncated_u = sum(CBF(-alpha/2)^(j-1) * w^j for j in range(1, order+1)) + CBF(0).add_error(bound_error_u) * w^(order+1)
        f_z = truncate_tail(f.subs({u : truncated_u}), deg, min_n, w)
        g = truncated_logder(alpha, l, order, v, logz, min_n) #Bound for [(d/dα)^l (Γ(n+α)/Γ(α)Γ(n+1))] / (Γ(n+α)/Γ(α)Γ(n+1))
        bound_error_v = CBF(abs(alpha)^order / (1 - 1/s))
        truncated_v = sum(CBF(-alpha)^(j-1) * w^j for j in range(1, order+1)) + CBF(0).add_error(bound_error_v) * w^(order+1)
        bound_error_logz = CBF(log(2)+1/2) * CBF(2*abs(alpha)).pow(order+1) / CBF(1 - 2/s)
        truncated_logz = logn - sum(CBF(-alpha)^j * w^j / j for j in range(1, order+1)) + CBF(0).add_error(bound_error_logz) * w^(order+1)
        g_z = truncate_tail(g.subs({v : truncated_v, logz : truncated_logz}), deg, min_n, w)
        h_z = truncated_power(alpha, order, w, s) #Bound for (1 + α/2n)^(α-1) = (n+α/2)^(α-1) * n^(1-α)
        ...
        poly_rec_1 = bound_coeff_mono(alpha + 1, l, deg, u, logz, s, min_n - 1) #|alpha| decreases, so n >= s*|alpha| still holds
        poly_rec_2 = bound_coeff_mono(alpha + 1, l - 1, deg, u, logz, s, min_n - 1)
        #u = 1/(n-1)
        bound_error_u = CBF(1 / (1 - 1/(min_n - 1)))
        truncated_u = sum(CBF(1) * w^j for j in range(1, order+1)) + CBF(0).add_error(bound_error_u) * w^(order+1)
        bound_error_logz = CBF(abs(log(2) * 2^(order+1)) / (1 - 2/(min_n - 1)))
        truncated_logz = logn - sum(CBF(1) * w^j / j for j in range(1, order+1)) + CBF(0).add_error(bound_error_logz) * w^(order+1)
        ss = CBF(alpha) * poly_rec_1.subs({u : truncated_u, logz : truncated_logz}) + CBF(l) * poly_rec_2.subs({u : truncated_u, logz : truncated_logz})
        return truncate_tail(ss, deg, min_n, w)

def _coeff_zero(seqini, deq):
    """
    Find coefficients of generating function in the basis with these expansions at the origin
    ...
    - coeff : vector, coefficients of generating function in the basis with these expansions at the origin
    """
...
def extract_local(fs, rho, order = 1):
    """
    Extract the local terms of a formal series of logmonomials, return a polynomial in L=logz, Z=z
    ...
    OUTPUT:
        ...
        - loc : element of same polynomial ring, local truncation (after variable change) of fs
    """
    PolLZ.<L, Z> = CBF[]
    mylog = CBF(-rho).log() - L
    val = valuation_FS(fs)
    loc = sum(c * mylog^mon.k * Z^mon.shift for c, mon in fs if mon.n < val + order)
    return L, Z, loc
...
def numerical_sol_big_circle(coeff_zero, deq, list_dom_sing, rad, halfside):
    ...
    for j in range(num_sings - 1):
        ...
        num_sq = num_sq + np
        circle_upper = [(hub*C(halfarc*k/np*I).exp()).add_error(halfside) for k in range(np+1)]
        circle_lower = [(hub*C(-halfarc*k/np*I).exp()).add_error(halfside) for k in range(np+1)]
        path_upper = [0] + [[_z] for _z in circle_upper]
        path_lower = [0] + [[_z] for _z in circle_lower]
        pairs = pairs + deq.numerical_solution(coeff_zero, path_upper, 1e-30, assume_analytic=True)
        pairs = pairs + deq.numerical_solution(coeff_zero, path_lower, 1e-30, assume_analytic=True)
...
def contribution_single_singularity(coeff_zero, deq, rho, rad_input, coord_big_circle, total_order = 1, min_n = 50):
    """
    ...
    - list_val_kappa : list of pairs (valuation of solutions at rho, upper bound powers of log terms)
    - list_lower_bound : list of expressions of lower bound for integral when n>monotone_bound
    - monotone_bound : list of integer, after which the bound for f_n * rho^n * n^(v+1) becomes monotone (but not necessarily positive!)
    """
    ...
    coord_all = deq.numerical_transition_matrix([0, rho], 1e-20, assume_analytic=True) * coeff_zero
    ...
    list_ind = [[ind for ind, x in enumerate(list_expo) if x == expo] for expo in list_expo_unique]
    ...
    R.<v, logz, u, w, logn> = PolynomialRing(CBF, order='lex')
    ...
    for ind_basis in list_ind:
        ...
        order = max(0, ceil(total_order - (val_rho - min_val_rho)))
        ...
        # Local expansion of f at z=rho, in terms of variables Z and L
        L, Z, locf_ini_terms = extract_local(locf_long, rho)
        ...
        K_ext.<val_rho1> = K.extension(QQbar(val_rho).minpoly())
        rho1 = K_ext(rho1)
        ...
        # Convert the equation to an enriched data structure necessary for calling
        # some of the bound computation routines, and shift to the origin.
        ...
        maj = DiffOpBound(ldop, leftmost=val_rho1, pol_part_len=30, bound_inverse="solve")

        # We want to bound the series h0, h1, ..., h_kappa on small disk |z - ρ| < smallrad.
        # The bounds that ore_algebra computes are too coarse, so we bound the tails
        # corresponding to a truncation at index order1 > order, and handle the
        # intermediate terms separately.
        ...
        coeff = { (mon.shift, mon.k): c*ZZ(mon.k).factorial() for c, mon in locf1 } #(z^shift*log^k)
        ...
        # Majorant series of [the components of] the tail of the local expansion of f
        # at ρ. See [Mez19, Sec. 4.3] and [Mez19, Algo. 6.11].
        #
        ...
        # Bound on the *values* for |z-ρ| <= smallrad of the functions h0, h1, 2*h2.
        vb = tb + ib
        ...
        ll = abs(CBF(-rho).log())
        B = vb*RBF['z']([sum([ll^(m - j) * binomial(m, j) / factorial(m) for m in range(j, kappa + 1)]) for j in range(kappa + 1)])

        s = floor(min_n / (abs(val_rho) + abs(order)))
        assert s>2, "min_n too small! Cannot guarantee s>2"

        # Sub polynomial factor for bound on S(n)
        cst_S = CBF(0).add_error(CBF(abs(rho)).pow(val_rho.real()+order) * ((abs(CBF(rho).arg()) + 2*RBF(pi))*abs(val_rho.imag())).exp() * CBF(1 - 1/min_n).pow(CBF(-min_n-1)))
        bound_S = cst_S*B(CBF(pi)+logn)
        # Sub polynomial factor for bound on L(n)
        if val_rho + order <= 0:
            C_nur = 1
        else:
            C_nur = 2 * (CBF(e) / (CBF(val_rho.real()) + order) * (s - 2)/(2*s)).pow((RBF(val_rho.real()) + order))
        cst_L = CBF(0).add_error(C_nur * CBF(1/pi) * CBF(abs(rho)).pow(RBF(val_rho.real())+order)) * ((abs(CBF(rho).arg()) + 2*RBF(pi))*abs(val_rho.imag())).exp()
        bound_L = cst_L*B(CBF(pi)+logn)

        # Values of the tail of the local expansion.
        # With the first square (and possibly some others), the argument of the log
        # that we substitute for L crosses the branch cut. This is okay because the
        # enclosure returned by Arb takes both branches into account.

        _zeta = CBF(rho)
        dom_big_circle = [(_z-_zeta).pow(CBF(val_rho)) * locf_ini_terms((~(1-_z/_zeta)).log(), _z-_zeta) for _z in coord_big_circle]
        val_big_circle = [val_big_circle[j] + dom_big_circle[j] for j in range(len(dom_big_circle))]

        list_coef_deg = [(c, mon.degree(L), mon.degree(Z)) for c, mon in list(locf_ini_terms)]

        bound_lead_terms = sum(tup[0] * CBF(- rho).pow(CBF(val_rho+tup[2])) * w^(tup[2]) * bound_coeff_mono(-val_rho-tup[2], tup[1], order - tup[2], w, logn, s, min_n) for tup in list_coef_deg)
        bound_int_SnLn = (bound_S + bound_L) * w^order

        list_bound.append(bound_lead_terms + bound_int_SnLn)

        cycle_end_time = time.time()
        print("Computing of basis %d finished, time: %9.2f" % (num_bas, cycle_end_time - cycle_begin_time))

    return list_val, list_bound, val_big_circle

def contribution_all_singularity(seqini, deq, singularities = None, known_analytic = [0], rad = None, total_order = 1, min_n = 50, halfside = None):
    """
    Compute a bound for the n-th element of a holonomic sequence

    INPUT:
        - seqini : list, initial elements of sequence, long enough to determine the sequence
        - deq : a linear ODE that the generating function satisfies
        - singularities : list of algebraic numbers, dominant singularities. If None, compute automatically
        - known_analytic : list of points where the generating function is known to be analytic, default is [0]
    ...
    OUTPUT:
        - bound : symbolic expression in variable 'n', a bound for the n-th element of a holonomic sequence
    """
    coeff_zero = _coeff_zero(seqini, deq)

    if singularities is None:
        desing_deq = deq.desingularize()
        list_sing = desing_deq.leading_coefficient().roots(QQbar, multiplicities=False)
    else:
        list_sing = singularities.copy()

    list_exception = deq.leading_coefficient().roots(QQbar, multiplicities=False) + [0]
    ...
    if rad is None:
        ...
        max_smallrad = min(min(abs(ex - ds) for ex in list_exception if not (ex - ds) == 0) for ds in list_dom_sing)
        rad_input = min(abs(list_sing[k])*0.9 + abs(list_sing[0])*0.1, abs(list_sing[0]) + max_smallrad*0.8) #This should probably change to avoid smaller fake singularities
    ...
    if len(list_dom_sing) > 1:
        min_dist = min(min(abs(ex - ds) for ex in list_dom_sing if not (ex - ds) == 0) for ds in list_dom_sing)
        assert min_n > ceil(2*abs(list_dom_sing[-1])/min_dist), "Please increase min_n to at least "+str(ceil(2*abs(list_dom_sing[-1])/min_dist))
    ...
    pairs = numerical_sol_big_circle(coeff_zero, deq, list_dom_sing, rad_input, halfside)
    ...
    for rho in list_dom_sing:
        list_val, list_bound, val_big_circle = contribution_single_singularity(coeff_zero, deq, rho, rad_input, coord_big_circle, total_order, min_n)
        bound = bound + sum([SR(QQbar(1/rho)^n) * SR(n^QQbar(-val-1)) * poly_bound(0,0,0, 1/n, log(n)) for val, poly_bound in zip(list_val, list_bound)])
        list_val_bigcirc.append(val_big_circle)
    sum_g = [sum(v) for v in zip(*list_val_bigcirc)]
    for v in list_val_bigcirc:
        max_big_circle = max_big_circle.max(*((s - vv).above_abs() for s, vv in zip(sum_g, v)))
    max_big_circle = max_big_circle.max(*((s - vv).above_abs() for s, vv in zip(sum_g, f_big_circle)))
    bound = bound + CBF(0).add_error(max_big_circle) * CBF(1/rad_input)^n
    return bound
...
# Find the desingularized equation
desing_deq = deq.desingularize()
desing_deq.leading_coefficient().factor()
# Potential singularities
rts = desing_deq.leading_coefficient().roots(QQbar, multiplicities=False)
rho = rts[1]
#result = contribution_single_singularity(seqini, deq, rts[1], 0.297, total_order = 1, min_n = 100)
# coeff_zero = vector([0,0,72])
# pairs = numerical_sol_big_circle(coeff_zero, deq, [rho], 0.297)
# coord_big_circle = [z for z, _ in pairs]
# contribution_single_singularity(coeff_zero, deq, rho, 0.297, coord_big_circle, total_order = 1, min_n = 50)
b = contribution_all_singularity(seqini, deq, total_order = 2)
...
seqini = [1]
deq = (4*z^4 - 4*z^3 + z^2 - 2*z + 1)*Dz + (-4*z^3 + 4*z^2 - z - 1)
# Find the desingularized equation
desing_deq = deq.desingularize()
desing_deq.leading_coefficient().factor()
# Potential singularities
b = contribution_all_singularity(seqini, deq, total_order = 5)
...
seqini = [1, -3, 9, -3, -279, 2997]
deq = z^2*(81*z^2 + 14*z + 1)*Dz^3 + 3*z*(162*z^2 + 21*z + 1)*Dz^2 + (21*z + 1)*(27*z + 1)*Dz + 3*(27*z + 1)
# Find the desingularized equation
desing_deq = deq.desingularize()
desing_deq.leading_coefficient().factor()
# Potential singularities
