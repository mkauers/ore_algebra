...
def factor(dop, verbose=False):
    ...
    :meth:`~ore_algebra.differential_operator_1_1.UnivariateDifferentialOperatorOverUnivariateRing.factor`.
    """
...
def right_factor(dop, verbose=False):
    r"""
    See the documentation of the associated method
    :meth:`~ore_algebra.differential_operator_1_1.UnivariateDifferentialOperatorOverUnivariateRing.right_factor`.
    """
...
def right_factor_via_monodromy(dop, order=None, bound=None, alg_degree=None, precision=None, loss=0, verbose=False):
    r"""
    Same as :func:`right_factor` but focused on the hybrid algorithm.
...
def right_factor_when_monodromy_is_trivial(dop, order, verbose=False):
    r"""
    Same as :func:`right_factor_via_monodromy` but focused on the case where the
    monodromy group is generated by homotheties.
...
def one_dimensional_eigenspaces(dop, mono, order, bound, alg_degree, verbose=False):
    ...
    A nontrivial right factor of ``dop``, or ``None``, or ``NotGoodConditions``,
    or ``Inconclusive``
    """
...
def multiple_eigenvalue(dop, mono, order, bound, alg_degree, verbose=False):
    ...
    A nontrivial right factor of ``dop``, or ``None``, or ``Inconclusive``
    """
    r = dop.order()
...
def annihilator(dop, ic, order, bound, alg_degree, mono=None, verbose=False):
    ...
    ``dop``, or a nontrivial right factor of ``dop``, or ``Inconclusive``
    """
...
def _try_rational(dop):
    r"""
    Return a right-hand factor of ``dop`` when it has rational solutions.
    ...
    Either a first-order right-hand factor of ``dop`` or ``None``.
...
def _transition_matrix_for_adjoint(dop):
    """
    Return an invertible constant matrix ``Q`` such that: if ``M`` is the
    monodromy of ``dop`` along a loop ``gamma``, then the monodromy of the
    adjoint of dop along ``gamma^{-1}`` is equal to ``Q*M.transpose()*(~Q)``,
    where the monodromies are computed in the basis given by the method
    :meth:`~ore_algebra.differential_operator_1_1.UnivariateDifferentialOperatorOverUnivariateRing.local_basis_expansions`.

    ASSUMPTIONS:
    
    ``dop`` is monic, ``0`` is the base point, and ``0`` is not singular.
    """
...
def _guess_symbolic_coefficients(vec, alg_degree, verbose=False):
    ...
    and its field of coefficients if something reasonable is found, or the word
    ...
    - ``K`` -- QQ, or a number field, or ``None``
...
def _formal_finite_sum_to_power_series(f, PSR):
    ...
    ``x`` is extended at 0 (you have to shift otherwise).
    """
...
def _substitution_map(dop, e):
    ...
    This corresponds to multiply the solution space by z^(-e).
    
    REFERENCES:
    
    - Factorization of Linear Differential Operators, van Hoeij, 1996
