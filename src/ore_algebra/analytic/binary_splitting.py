...
class MatrixRec(object):
    ...
    def __init__(self, diffop, dz, derivatives):
        ...
        self._eval_template = StepMatrix(
                MatrixSpace(AlgInts_rec, self.ordrec, self.ordrec).zero_matrix(),
                1,
                pow_num,
                pow_den,
                MatrixSpace(Series_sums, 1, self.ordrec).zero_matrix())
    ...
    def __call__(self, n):
        ...
        stepmat.sums_row[0, self.orddiff] = stepmat.rec_den * stepmat.pow_den
        return stepmat
    ...
    def unroll(self, low, high):
        return self.binsplit(low, high)
...
def _number_field_with_integer_gen(K):
    if K is QQ:
        ...
        intgen = K.gen() * K.defining_polynomial().denominator()
        intNF = NumberField(intgen.minpoly(), str(intgen), embedding=intgen)
