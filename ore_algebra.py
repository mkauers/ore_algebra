...
if datetime.date(int(v[-10:-6]), int(v[-5:-3]), int(v[-2:])) >= datetime.date(2013, 05, 15):
    try:
        load("shift.spyx") # cython code defining 'taylor_shift_univ_int_poly'
    except:
        pass
                ...
                if sx - x == one:
                    ...
                    if R.base_ring() is ZZ and R.ngens() == 1:
                        if R.is_field():
                            ...
                            gens[i][1].inverse().set_call(lambda q, k: taylor_shift_univ_int_ratfun(R(q), -k))
                        else:
                            gens[i][1].set_call(lambda p, k=1: taylor_shift_univ_int_poly(R(p), k))
                            gens[i][1].inverse().set_call(lambda p, k: taylor_shift_univ_int_poly(R(p), -k))
                elif gens[i][1](sx)*x == sx**2:
                    is_qshift[i] = True
            elif dx == x:
                if sx == x:
